# handler.py - Versión SFT para acestep_v1.5_xl_sft_bf16
import runpod
import json
import requests
import time
...
def log_system_info():
    ...
    if os.path.exists("/runpod-volume"):
        ...
        if os.path.exists("/runpod-volume/models"):
            ...
            for subdir in ["diffusion_models", "text_encoders", "checkpoints", "unet", "vae", "clip"]:
...
def check_models():
    ...
    # ── Checkpoint principal → diffusion_models/ ──────────────────
    for base in base_paths:
        path = f"{base}/diffusion_models/{REQUIRED_MODELS['checkpoint']}"
        if os.path.exists(path):
            found_models['checkpoint'] = path
            break
    else:
        missing_models.append(f"diffusion_models/{REQUIRED_MODELS['checkpoint']}")

    # ── CLIP / text encoders → text_encoders/ ─────────────────────
    for clip_key in ["clip_1", "clip_2"]:
        for base in base_paths:
            path = f"{base}/text_encoders/{REQUIRED_MODELS[clip_key]}"
            if os.path.exists(path):
                found_models[clip_key] = path
                break
        else:
            missing_models.append(f"text_encoders/{REQUIRED_MODELS[clip_key]}")

    # ── VAE → vae/ ────────────────────────────────────────────────
    for base in base_paths:
        path = f"{base}/vae/{REQUIRED_MODELS['vae']}"
        if os.path.exists(path):
            found_models['vae'] = path
            break
    else:
            ...
            if history_response.status_code == 200:
                ...
                if prompt_id in history:
                    ...
                    if audio_info:
                        ...
                        if error:
                            return {"error": f"Error descargando audio: {error}", "audio_info": audio_info}
